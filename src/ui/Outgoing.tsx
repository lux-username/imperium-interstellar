/**
 * Dispatches the player has sent. Their fate is unknown: the desk sees only
 * when they left and when the timetable says they should land.
 */
import type { PlayerView, WorldId } from '../sim/view'
import { weekLabel, worldName } from './format'
      ...
      {view.outgoing.map((d) => {
        ...
        return (
          <li key={d.id} onClick={() => to && onSelect(to)}>
            ...
            <div className="line2">
              {d.payload.kind === 'letter' ? 'Letter' : d.payload.kind} · sent {weekLabel(d.envelope.sent)} · expected {d.envelope.eta !== null ? weekLabel(d.envelope.eta) : 'never'} · via{' '}
